...
def check_trajectory(ttc, a_comb, bound_collision, ttc_treshold=2.0, a_comb_threshold=15.0):
    ...
    if ttc <= ttc_treshold:
        safe = 0
    if max(a_comb) > a_comb_threshold:
        safe = 0
    if bound_collision is True:
        safe = 0
...
def check_bound_collision(path, bound, veh_width=2.8):
    ...
    Path = shapely.geometry.LineString(path)
    Bound = shapely.geometry.LineString(bound)
    ShapeOfPath = Path.buffer(veh_width / 2)
    intersection = ShapeOfPath.intersects(Bound)
