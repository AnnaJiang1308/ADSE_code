...
def get_aabb_polygon(trajectory: dict):
    ...
    #  Start of your code  #
    ########################
    Vehs = unary_union(trajectory["vehicle_polygons"])
    x = Vehs.bounds
    xmin = x[0]
    ymin = x[1]
    xmax = x[2]
...
def calculate_mbc_radius(rectangle_width: float, rectangle_length: float):
    ...
    #  Start of your code  #
    ########################
    radius = np.sqrt((rectangle_length / 2)**2 + (rectangle_width / 2)**2)
...
def check_circle_collision(xy1: np.ndarray, xy2: np.ndarray, r: float):
    ...
    circle_collision = bool(np.sqrt(np.sum((xy1 - xy2)**2)) <= 2 * r)
...
def polygon_collision_check(trajectory1: dict, trajectory2: dict):
    ...
    Veh1 = trajectory1["vehicle_polygons"]
    Veh2 = trajectory2["vehicle_polygons"]
    polygon_collision = False
    for i in range(len(Veh1)):
        if Veh1[i].intersects(Veh2[i]):
...
def mbc_collision_check(trajectory1: dict, trajectory2: dict, mbc_radius: float,
    ...
    for i in range(len(XY1)):
        if given_circle_collision_check(XY1[i], XY2[i], mbc_radius):
            mbc_collision = True
    ########################
    #   End of your code   #
    ########################
